...
const academicItems = [
  { icon: '📄', label: 'Transcript of Records', sub: 'Official digital copy (e-TOR)',    path: '/profile/edit' },
  { icon: '👥', label: 'Committees & Orgs',      sub: 'SAMAHAN, BKV (Active)',             path: '/profile/edit' },
  { icon: '💝', label: 'Past Donations',          sub: 'Blue Knight Scholars Fund',        path: '/donations/history' },
];
    ...
    <MobileShell>
      ...
      <div className="bg-white pb-6 flex flex-col items-center pt-8 border-b border-gray-100">
        ...
        <div className="flex gap-6 mt-5">
          {[['₱12,500','Donated'],['8','Campaigns'],['2024','Batch']].map(([v, l]) => (
      ...
      <div className="px-4 py-5 space-y-4">
        ...
        >
          ...
          <div className="flex-1">
            <p className="font-semibold text-amber-900 text-sm">Certificate Vault</p>
            <p className="text-xs text-amber-700 mt-0.5">View & download your impact certificates</p>
          </div>
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden divide-y divide-gray-100">
            {accountItems.map((item, i) => (
              ...
              >
                ...
                <span className={`flex-1 font-semibold text-sm ${item.danger ? 'text-red-500' : 'text-gray-800'}`}>
                  {item.label}
                </span>
                {!item.danger && <span className="text-gray-300 text-lg">›</span>}
              </button>
            ))}
          </div>
        </div>
        ...
        <button
          ...
          className="w-full flex items-center gap-4 bg-brand-50 border border-brand-200 rounded-2xl p-4 text-left hover:bg-brand-100 transition-colors"
        >
          <div className="w-10 h-10 rounded-xl bg-brand-100 flex items-center justify-center text-xl flex-shrink-0">
            🏆
          </div>
          <div className="flex-1">
            <p className="font-semibold text-brand-900 text-sm">Batch Leaderboard</p>
            <p className="text-xs text-brand-600 mt-0.5">See how your batch ranks</p>
          </div>
