...
const StatCard = ({ icon, label, value, sub, color }) => (
  <div className="bg-admin-800/60 rounded-2xl p-4 border border-admin-700">
    ...
    <p className={`font-black text-xl mt-2 ${color || 'text-white'}`}>{value}</p>
    <p className="text-admin-300 text-xs uppercase tracking-wide font-semibold mt-0.5">{label}</p>
    {sub && <p className="text-admin-400 text-[10px] mt-0.5">{sub}</p>}
  </div>
);

const newsItems = [
  { title: 'Grand Alumni Homecoming 2024', date: 'OCT 24, 2024', color: 'from-teal-400 to-teal-700' },
  { title: 'Scholarship Goal',             date: 'OCT 12, 2024', color: 'from-gray-600 to-gray-900' },
];
    ...
    <MobileShell>
      ...
      <div className="bg-gradient-to-br from-admin-900 via-admin-800 to-admin-700 px-5 py-5">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-admin-300 text-xs font-medium uppercase tracking-wide">Welcome, Admin!</p>
            <p className="text-white font-black text-lg leading-tight">Manon Bannerman</p>
          </div>
        </div>
      ...
      <div className="bg-gradient-to-b from-admin-800 to-admin-900 px-4 pt-4 pb-5">
        <div className="grid grid-cols-2 gap-3">
          ...
          <div className="bg-admin-800/60 rounded-2xl p-4 border border-admin-700">
            ...
            <p className="text-admin-300 text-xs uppercase tracking-wide font-semibold mt-2">Active Campaigns</p>
      ...
      <div className="px-4 pt-4 pb-2">
        ...
        <div className="grid grid-cols-2 gap-3">
          {newsItems.map((item, i) => (
            <button key={i} className="rounded-2xl overflow-hidden shadow-sm aspect-[4/3]">
              <div className={`w-full h-full bg-gradient-to-br ${item.color} flex flex-col justify-end p-3`}>
                <p className="text-white font-semibold text-xs leading-tight text-left">{item.title}</p>
                <p className="text-white/70 text-[10px] mt-0.5 text-left">{item.date}</p>
              </div>
            </button>
          ))}
        </div>
