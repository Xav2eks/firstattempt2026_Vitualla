...
export default function BatchLeaderboardPage() {
  ...
  return (
    <MobileShell>
      ...
      {/* Tabs */}
      <div className="bg-admin-800 px-4 pb-3">
                ...
                >
                  ...
                  <div
                    className={`mt-2 rounded-t-xl flex flex-col items-center justify-end pb-2 px-3 ${
                      isFirst ? "h-24 bg-admin-800" : "h-16 bg-admin-700"
                    }`}
                    style={{ minWidth: "72px" }}
                  >
                    <p className="text-white font-black text-sm">{b.raised}</p>
                    <p className="text-brand-300 text-[9px] font-semibold uppercase">
                      Raised
                    </p>
              ...
              >
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-2">
                    ...
                    {item.isYours && (
                      <span className="bg-admin-600 text-white text-[9px] font-black px-2 py-0.5 rounded-full uppercase tracking-wide">
                        Your Batch
                      </span>
                    )}
                  </div>
                  ...
                  <div className="mt-1.5 bg-gray-100 rounded-full h-1.5 w-full">
                    <div
                      className={`h-1.5 rounded-full ${item.isYours ? "bg-admin-600" : "bg-gray-300"}`}
                      style={{ width: item.participation }}
                    />
                  </div>
                </div>
                <button
                  className={`flex-shrink-0 px-3 py-2 rounded-xl text-xs font-bold flex items-center gap-1 ${
                    item.isYours
                      ? "bg-brand-100 text-admin-700 hover:bg-brand-200"
        ...
        <div className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <p className="font-black text-2xl text-gray-900 mt-0.5">
                42 Classes
              </p>
            </div>
          </div>
